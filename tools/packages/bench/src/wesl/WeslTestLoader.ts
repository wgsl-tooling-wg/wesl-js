...
import type { TestLoader } from "../TestLoader.ts";
import { loadSimpleTest, loadSimpleFiles } from "./LoadSimpleTest.ts";
...
/** WESL-specific test loader implementation */
export class WeslTestLoader implements TestLoader {
  async loadTests(config: BenchConfig): Promise<BenchTest<any>[]> {
    const weslExt = getWeslExtension(config);
    
    if (weslExt.testSource === 'simple') {
      return this.loadSimpleBenchTests(config, weslExt.simpleTestName);
    } else {
      return setupWeslBenchmarks(config.filter, weslExt.variants, config.useBaseline);
    }
  }
  
  /** Load simple benchmark tests */
  private async loadSimpleBenchTests(
    config: BenchConfig, 
    ...
    const benchTest: BenchTest<Record<string, string>> = {
      ...
      metadata: {
        weslBenchTest,
        linesOfCode: this.calculateLinesOfCode(weslSrc),
      },
    };
  ...
  private calculateLinesOfCode(weslSrc: Record<string, string>): number {
