...
test("@texture single resolves <canvas data-texture> child", async ({
  ...
  await page.goto("/");
  await waitForFrame(page, "#player16");
  await expectCanvasSnapshot(page, "#player16", "texture-grid.png");
});

test("@texture multi blends two host canvases", async ({ page }) => {
  await page.goto("/");
  await waitForFrame(page, "#player17");
  await expectCanvasSnapshot(page, "#player17", "texture-multi.png");
});
...
test("@buffer zero-init storage buffer renders deterministically", async ({
  ...
  await page.goto("/");
  await waitForFrame(page, "#player18");
  await expectCanvasSnapshot(page, "#player18", "buffer-zero-init.png");
});
...
test("@texture from <img> decodes and rebuilds on src swap", async ({
  ...
  await page.goto("/");
  await waitForFrame(page, "#player20");
  await expectCanvasSnapshot(page, "#player20", "texture-img-magenta.png");

  await page.click("#swap-img20");
  await waitForNewFrame(page, "#player20");
  await expectCanvasSnapshot(page, "#player20", "texture-img-green.png");
});
...
test("@test_texture is rejected with structured compile-error", async ({
  ...
  await page.waitForFunction(
    () =>
      (document.querySelector("#reject19-status") as HTMLElement)?.dataset
        .rejected === "true",
  );

  const text = await page.textContent("#reject19-status");
...
test("no critical console errors on basic load", async ({ page }) => {
  ...
  const criticalErrors = errors.filter(
    e =>
      !e.includes("favicon") &&
      !e.includes("404") &&
      // section 19 deliberately demonstrates the @test_texture rejection on load
      !e.includes("@test_texture"),
  );
