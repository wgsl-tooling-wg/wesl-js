...
function handleRef(
  ...
  if (!ident.refersTo && !ident.std) {
    const foundDecl = findDeclInModule(ident, liveDecls) ??
...
function failMissingIdent(ident: RefIdent): void {
  ...
  if (refIdentElem) {
    ...
    const msg =
      `unresolved identifier '${ident.originalName}' in file: ${filePath}`; // TODO make error message clickable
...
function findQualifiedImport(
  ...
  const modulePathParts = matchingImport(identParts, flatImps) ??
    qualifiedImport(identParts);
...
function findExport(
  ...
  const moduleAst = parsed.modules[modulePath] ??
