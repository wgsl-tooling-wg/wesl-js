import { srcLog, type SrcMapBuilder } from "@wesl/mini-parse";
import type {
  AbstractElem,
  AttributeElem,
  ContainerElem,
...
export function expressionToString(elem: ExpressionElem): string {
  ...
  if (kind === "binary-expression") {
    return `${expressionToString(elem.left)} ${elem.operator.value} ${
      expressionToString(elem.right)
    }`;
    ...
    return `${elem.function.ident.originalName}(${
