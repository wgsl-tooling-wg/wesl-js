...
function filterIfAttributes(
  ...
  return filterMap(
    attributes,
    (a) => a.attribute.kind === "@if" ? a.attribute : undefined,
  );
}
...
function fnTags(cc: CollectContext) {
  ...
  const returnAttributes: AttributeElem[] | undefined = return_attributes
    ?.flat();
...
export function logCollect(msg?: string): (cc: CollectContext) => void {
  return function _log(cc: CollectContext) {
    console.log(msg ?? "log", { tags: [...Object.keys(cc.tags)] });
  };
}
...
export const switchClauseCollect = attrElemCollect<SwitchClauseElem>(
  "switch-clause",
);
