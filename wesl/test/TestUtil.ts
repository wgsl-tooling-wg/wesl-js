import { type Parser, type Stream, withLoggerAsync } from "@wesl/mini-parse";
import {
  expectNoLog,
  logCatch,
...
import { bindAndTransform, link, type LinkParams } from "../Linker.ts";
...
/** parse a single wesl file, returning the parse state as well as the WeslAST */
 ...
 */
export function linkTest(...rawWgsl: string[]): Promise<string> {
...
export function linkWithLog(...rawWgsl: string[]): Promise<{
  log: string;
  result: string;
}> {
...
export function linkWithLogQuietly(...rawWgsl: string[]): Promise<{
  log: string;
  result: string;
}> {
