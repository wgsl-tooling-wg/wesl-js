...
function discoverScripts(
  ...
  const toolsScripts = loadPackageScripts(rootPackagePath, "tools");
  ...
  if (existsSync(localPackagePath) && localPackagePath !== rootPackagePath) {
...
function runScript(
  ...
  if (source === "agent" && path) {
    ...
    runPnpmScript(name, args, repoRoot);
  }
}
...
function showHelp(scripts: Map<string, ScriptInfo>): void {
  ...
  printGroup(filterBySource(scripts, "tools"), "Root (from package.json)");
